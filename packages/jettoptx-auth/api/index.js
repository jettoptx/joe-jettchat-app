...
function base64UrlEncode(data) {
  const str = typeof data === "string" ? data : (0, import_tweetnacl_util.encodeBase64)(data);
  return str.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
function base64UrlDecode(str) {
  const padded = str.replace(/-/g, "+").replace(/_/g, "/");
  return (0, import_tweetnacl_util.decodeBase64)(padded + "=".repeat((4 - padded.length % 4) % 4));
}
...
function signJWT(claims, privateKey) {
  ...
  const signature = import_tweetnacl.default.sign.detached(message, privateKey);
...
function verifyJWT(token, publicKey) {
  ...
  if (!import_tweetnacl.default.sign.detached.verify(message, signature, publicKey)) {
...
var import_tweetnacl, import_tweetnacl_util, HEADER;
var init_jwt = __esm({
  "src/jwt.ts"() {
    "use strict";
    import_tweetnacl = __toESM(require("tweetnacl"));
    import_tweetnacl_util = require("tweetnacl-util");
...
__export(api_exports, {
  createRefreshHandler: () => createRefreshHandler,
  createWalletChallengeHandler: () => createWalletChallengeHandler,
  createWalletVerifyHandler: () => createWalletVerifyHandler,
  createXAuthHandler: () => createXAuthHandler,
  createXCallbackHandler: () => createXCallbackHandler,
...
function createNonceChallenge(walletPubkey) {
  const nonce = Array.from(import_tweetnacl2.default.randomBytes(32)).map((b) => b.toString(16).padStart(2, "0")).join("");
...
function verifySignedMessage(message, signatureBase64, walletPubkeyBase58) {
  try {
    ...
    const signatureBytes = (0, import_tweetnacl_util2.decodeBase64)(signatureBase64);
    const pubkeyBytes = base58Decode(walletPubkeyBase58);
    const valid = import_tweetnacl2.default.sign.detached.verify(
      messageBytes,
      signatureBytes,
      pubkeyBytes
    );
